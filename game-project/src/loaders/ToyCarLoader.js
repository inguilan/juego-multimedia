...
export default class ToyCarLoader {
    ...
    _processBlocks(blocks, precisePhysicsModels) {
        blocks.forEach(block => {
            ...
            if (glb) {
                model = glb.scene.clone();
            } else {
                // Fallback: si es una moneda/coin, crear un modelo simple en runtime
                if (block.name && block.name.toLowerCase().startsWith('coin')) {
                    console.warn(`Modelo no encontrado para '${resourceKey}', usando fallback de moneda.`);
                    const group = new THREE.Group();
                    const geom = new THREE.CylinderGeometry(0.4, 0.4, 0.05, 24);
