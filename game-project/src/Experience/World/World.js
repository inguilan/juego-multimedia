...
export default class World {
    ...
    resetRobotPosition(spawn = { x: -17, y: 1.5, z: -67 }) {
        ...
        // Para evitar que el robot sea expulsado por solapamientos con cuerpos estáticos,
        // deshabilitamos temporalmente las colisiones del cuerpo mientras lo reposicionamos.
        const body = this.robot.body
        const originalMask = body.collisionFilterMask

        try {
            body.collisionFilterMask = 0 // no colisiona temporalmente
            ...
            // Forzar wakeUp tras reubicación
            setTimeout(() => {
                body.collisionFilterMask = originalMask
                try { body.wakeUp() } catch (e) { /* noop */ }
            }, 100)
        } catch (err) {
            console.warn('⚠️ Error al resetear posición del robot:', err)
            // Intento de fallback sin cambiar máscara
            body.position.set(spawn.x, spawn.y, spawn.z)
            body.velocity.set(0, 0, 0)
            body.angularVelocity.set(0, 0, 0)
            body.quaternion.setFromEuler(0, 0, 0)
