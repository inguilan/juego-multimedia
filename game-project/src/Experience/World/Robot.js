...
export default class Robot {
    ...
    setPhysics() {
        ...
        // Listener de colisiones para ayudar a diagnosticar empujes/teletransportes
        try {
            this.body.addEventListener('collide', (event) => {
                // Solo loguear cuando el modo debug esté activo
                if (!this.experience?.debug?.active) return

                const other = event.body || null
                try {
                    console.warn('🤖 Robot colisionó con:', other && (other.id || other.type || other.mass), {
                        robotPos: this.body.position,
                        robotVel: this.body.velocity,
                        otherPos: other ? other.position : null,
                        otherVel: other ? other.velocity : null
                    })
                } catch (err) {
                    console.warn('🤖 Robot collide (info parcial):', { robotPos: this.body.position })
                }
            })
        } catch (err) {
            // noop: algunos entornos no permiten addEventListener en bodies mock
        }

        // Inicializar posición segura (se actualizará durante el juego)
        try {
            this.lastSafePosition = this.body.position.clone()
    ...
    update() {
        ...
        // Limitar velocidad vertical para evitar impulsos excesivos que lancen al robot
        try {
            const maxYSpeed = 8
            const minYSpeed = -20
            this.body.velocity.y = Math.max(Math.min(this.body.velocity.y, maxYSpeed), minYSpeed)
        } catch (err) {
            // noop
        }
        ...
        // Detectar teleports bruscos (salto grande en una sola frame) y restaurar a última posición segura
        try {
            if (this.previousPosition) {
                const dist = this.body.position.distanceTo(this.previousPosition)
                if (dist > 8) {
                    console.warn('⚠️ Teleport detectado. Distancia:', dist)
                    if (this.lastSafePosition) {
                        console.warn('↩️ Restaurando a última posición segura:', this.lastSafePosition)
            ...
            // Actualizar posición segura si estamos cerca del suelo y con baja velocidad
            if (this.body.position.y < 3 && this.body.velocity.length() < 2) {
                try {
                    this.lastSafePosition = this.body.position.clone()
                } catch (e) {
                    // noop
            ...
            // Guardar previousPosition para próxima iteración
            try {
                this.previousPosition = this.body.position.clone()
            } catch (e) {
                // noop
